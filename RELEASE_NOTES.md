## v1.3.0 — the MCP instructions name the configured targets
...
new install — this server now reports `Configured targets: could not be read (FileNotFoundError) — run `vmware-vdi doctor`.` instead of falling silent. A client shown no listing cannot
...
Every CLI command that reaches Horizon now passes the same `guard()` as its MCP twin and writes one
audit row under that tool's name (15 commands, `@audited`). Commands that reach nothing remote say so, with a reason (1, `@cli_local`).
